::==============================================================================
:: hdl_designer.bat - Starts HDL designer
::
:start
...
if %REQUIRE_LIBERO% == 1 (
  ...
  set SYNTHESIS_HOME=%LIBERO_HOME%
)
...
set scripts_directory="!design_directory:"=!\Scripts"
set concat_directory="!design_directory:"=!\Board\concat"
...
set SCRIPTS_DIR=%scripts_directory:"=%
...
set SYNTHESIS_BASE_DIR=%design_directory:"=%\%synthesis_subdirectory:"=%
set SYNTHESIS_WORK_DIR=%scratch_directory:"=%\%DESIGN_NAME:"=%\%synthesis_subdirectory:"=%
...
if !VERBOSE! == 1 (
  ...
  echo "%INDENT:"=%SCRIPTS_DIR     is %SCRIPTS_DIR:"=%"
  ...
  if %REQUIRE_ISE% == 1 (
    ...
    echo "%INDENT:"=%ISE_BASE_DIR    is %SYNTHESIS_BASE_DIR:"=%"
    echo "%INDENT:"=%ISE_WORK_DIR    is %SYNTHESIS_WORK_DIR:"=%"
  )
  if %REQUIRE_LIBERO% == 1 (
    echo "%INDENT:"=%SYNTHESIS_HOME     is %SYNTHESIS_HOME:"=%"
    echo "%INDENT:"=%SYNTHESIS_BASE_DIR is %SYNTHESIS_BASE_DIR:"=%"
    echo "%INDENT:"=%SYNTHESIS_WORK_DIR is %SYNTHESIS_WORK_DIR:"=%"
  )
  echo.
)
...
if exist %SYNTHESIS_BASE_DIR% (
    echo "%SYNTHESIS_BASE_DIR:"=%"
    echo "  -> %SYNTHESIS_WORK_DIR:"=%"
    if exist %SYNTHESIS_WORK_DIR% (
	    rmdir /S /Q "%SYNTHESIS_WORK_DIR%"
    )
    mkdir "%SYNTHESIS_WORK_DIR%"
    xcopy /S /Y "%SYNTHESIS_BASE_DIR%" "%SYNTHESIS_WORK_DIR%\"
)


REM if %REQUIRE_ISE% == 1 (
  REM if exist %ISE_BASE_DIR% (
    REM echo "%ISE_BASE_DIR:"=%"
    REM echo "  -> %ISE_WORK_DIR:"=%"
    REM if exist %ISE_WORK_DIR% (
      REM rmdir /S /Q "%ISE_WORK_DIR%"
    REM )
    REM mkdir "%ISE_WORK_DIR%"
    REM xcopy /Y "%ISE_BASE_DIR%" "%ISE_WORK_DIR%\"
  REM )
REM )

REM if %REQUIRE_LIBERO% == 1 (
  REM if exist %LIBERO_BASE_DIR% (
    REM echo "%LIBERO_BASE_DIR:"=%"
    REM echo "  -> %LIBERO_BASE_DIR:"=%"
    REM if exist %LIBERO_WORK_DIR% (
      REM rmdir /S /Q "%LIBERO_WORK_DIR%"
    REM )
    REM mkdir "%LIBERO_WORK_DIR%"
    REM xcopy /S /Y "%LIBERO_BASE_DIR%" "%LIBERO_WORK_DIR%\"
  REM )
REM )
